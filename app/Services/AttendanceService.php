...
class AttendanceService
{
    ...
    public function getOrCreateRegister(array $data, int $userId): array
    {
        ...
        $sessionId = (int)$data['attendance_session_id'];
        $date      = $data['attendance_date'];

        $existing = $this->findExistingRegister($schoolId, $classId, $streamId, $sessionId, $date);
        if ($existing) {
            return $existing;
        }

        $id = $this->db->insert('attendance_registers', [
            'school_id'             => $schoolId,
            ...
            foreach ($students as $student) {
                $existing = $this->db->fetch(
                    "SELECT id FROM attendance_records
                     WHERE register_id = :register_id AND student_id = :student_id",
                ...
                if ($existing) {
                    ...
                    $recordData['id'] = $this->db->insert('attendance_records', $recordData);
                }
    ...
    public function getStudentAttendanceSummary(int $studentId, int $schoolId, array $filters = []): array
    {
        ...
        foreach ($records as $record) {
            if ($record['counts_as_present']) $present++;
            if ($record['counts_as_absent'])  $absent++;
            if ($record['counts_as_late'])    $late++;
        }
    ...
    private function findExistingRegister(
        int $schoolId,
        int $classId,
        ?int $streamId,
        int $sessionId,
