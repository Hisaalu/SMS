...
$nonGradedCount    = (int)  ($card['non_graded_count']    ?? 0);
$bestSubjects      = is_array($card['best_subjects'] ?? null) ? $card['best_subjects'] : [];
$bestSubjectsCount = (int)  ($card['best_subjects_count'] ?? $gradedSubjects);
...
<div class="report-card">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px;">
            ...
            <tbody>
                ...
                <tr style="background:<?= htmlspecialchars($tableHeadBg) ?>;font-weight:900;">
                    ...
                    <?php endforeach; ?>
                    <?php if ($showGrades): ?>
                        <td style="<?= $tdStyle ?>color:<?= htmlspecialchars($primaryColor) ?>;"><?= (int)round($totalAggregate) ?></td>
                    <?php endif; ?>
    ...
    <div class="no-print" style="margin-top:18px;text-align:right;font-family:Arial,sans-serif;border-top:1px solid #e0e0e0;padding-top:10px;">
        <a href="<?= BASE_URL ?>/reports/academic/report-cards" style="display:inline-block;padding:6px 14px;background:#546e7a;color:#fff;text-decoration:none;border-radius:3px;font-size:12px;margin-right:6px;font-weight:700;">&larr; Back</a>
        <a href="javascript:window.print()" style="display:inline-block;padding:6px 14px;background:<?= htmlspecialchars($primaryColor) ?>;color:#fff;text-decoration:none;border-radius:3px;font-size:12px;font-weight:700;">Print Report</a>
    </div>
</div>
</body>
</html>