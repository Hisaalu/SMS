...
<div class="container-fluid px-0">
    ...
    <div class="card p-4">
        <form method="POST" action="<?= BASE_URL ?>/academic/streams/<?= $stream->id ?>">
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <select name="class_id" id="class_id" class="form-select" required>
                        ...
                        <?php foreach ($classes as $c): ?>
                            <option value="<?= $c->id ?>" <?= ($stream->class_id == $c->id) ? 'selected' : '' ?>>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="name" id="name" class="form-control" value="<?= htmlspecialchars($stream->name ?? '') ?>" required>
                </div>

                <div class="col-12 mt-4">
                    <button type="submit" class="btn btn-primary">Update Stream</button>
                    <a href="<?= BASE_URL ?>/academic/streams" class="btn btn-secondary ms-2">Cancel</a>
                </div>
            </div>
        </form>
    </div>
